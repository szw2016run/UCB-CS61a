...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if len(current_place.bees) != 0 and self.min_range <= distance <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        #     self.digesting -= 1
        # else:
        #     digesting = time_to_didest
        if len(self.place.bees) != 0 and self.digesting == 0:
            bad_luck_bee = random.choice(self.place.bees)
            bad_luck_bee.reduce_armor(bad_luck_bee.armor)
